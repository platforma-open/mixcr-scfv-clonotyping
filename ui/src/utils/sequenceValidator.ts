...
export function validateSeparateChain(raw: string): SimpleValidation {
  ...
  if (!s.startsWith(">"))
    return { isValid: false, error: 'FASTA header is required. Start with ">" and a record name.' };
  const fv = validateFastaSequence(s);
  if (!fv.isValid)
    return {
      isValid: false,
      error: fv.error ?? (fv.errors ? fv.errors.join("\n") : "Invalid FASTA"),
    };
...
export function validateFullScFv(
  scfvRaw: string,
  linker: string,
...
export function validateLinker(raw: string): SimpleValidation | undefined {
  ...
  if (!/^[ACGT]+$/.test(clean))
    return { isValid: false, error: "Linker must contain only A/C/G/T characters" };
  return { isValid: true };
}
...
export function validateHinge(raw: string): SimpleValidation | undefined {
  ...
  const clean = s.toUpperCase().replace(/\s/g, "");
  if (!/^[ACGT]+$/.test(clean))
    return { isValid: false, error: "Hinge must contain only A/C/G/T characters" };
  return { isValid: true };
}

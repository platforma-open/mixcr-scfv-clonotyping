...
For the same reason, a custom-reference block that requests light-chain
imputation without a light-chain reference is refused: only the derived impute
sequence makes the workflow impute, while the request on its own stopped the
light tag pattern from being required, leaving the light chain neither extracted
nor imputed.
